...
const Header = ({ userFromDB, pfpLink }) => {
    ...
    const links = [
        ...
        ...(userFromDB?.role !== "manager" && userFromDB?.role !== "user"
            ? [
                {
                    icon: <FolderCog />,
                    title: "Projects",
    ...
    return (
        <header className='w-full px-8 py-2 md:py-9 flex bg-foreground items-center justify-between md:justify-end gap-4 sticky top-0 z-50 border-b border-background'>
